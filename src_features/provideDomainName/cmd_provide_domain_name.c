...
static uint32_t handle_first_chunk(const uint8_t **data, uint8_t *length, s_tlv_payload *payload) {
 ...
 */
uint32_t handle_provide_domain_name(uint8_t p1, const uint8_t *data, uint8_t length) {
    s_sig_ctx sig_ctx;
    uint32_t sw = APDU_RESPONSE_UNKNOWN;

    if (p1 == P1_FIRST_CHUNK) {
        sw = handle_first_chunk(&data, &length, &g_tlv_payload);
